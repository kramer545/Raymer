...
Shader "Flowhiz/Editor/PreviousWater" 
{
	Properties 
	{
		BaseColor("Horizon color", COLOR)  = ( .172 , .463 , .435 , 0)
		...
		WaveScale1("Wave scale", Float) = .07
		WaveScale2("Wave scale", Float) = .07
	...
	Category 
	{
		...
		SubShader 
		{
			Pass 
			{
				...
				uniform fixed4 FresnelColor;
				uniform half WaveScale1;
				uniform half WaveScale2;
				uniform float4 ReflactionMap_ST; 
				uniform half WaveStrange;

				uniform half EditorTime;
				uniform half _BrushSize;
				uniform half _BrushStrange;
				uniform float3 _BrushPosition;
				
				inline fixed SmoothStep(fixed edge0, fixed edge1, fixed x)
				{
				...
				{
					half time = _Time.x;
					#ifdef FLOW_EDITOR_PREVIEW_MODE
					time = EditorTime;
					#endif
					...
					#ifndef FLOW_EDITOR_PREVIEW_MODE
					float brushSize = length(i.flowUV - _BrushPosition) / (_BrushSize * 0.5h);
