...
public class FourierGPU
{
	...
	{	
		...
		Shader sx = Shader.Find("Ocean/FourierX");
		Shader sy = Shader.Find("Ocean/FourierY");
		
		if(sx == null) Debug.Log("FourierGPU::FourierGPU - Could not find shader Ocean/FourierX");
		if(sy == null) Debug.Log("FourierGPU::FourierGPU - Could not find shader Ocean/FourierY");
		
		m_fourierX = new Material(sx);
		m_fourierY = new Material(sy);
		...
		m_fourierX.SetFloat("_Size", m_fsize);
		m_fourierY.SetFloat("_Size", m_fsize);
	}
	...
	public int PeformFFT(RenderTexture[] data0, RenderTexture[] data1, RenderTexture[] data2)
	{
		...
		for(i = 0; i < m_passes; i++, j++) 
		{
			...
			m_fourierX.SetTexture("_ButterFlyLookUp", m_butterflyLookupTable[i]);
			
			m_fourierX.SetTexture("_ReadBuffer0", data0[idx1]);
			m_fourierX.SetTexture("_ReadBuffer1", data1[idx1]);
			m_fourierX.SetTexture("_ReadBuffer2", data2[idx1]);
			
			if(idx == 0)
				RTUtility.MultiTargetBlit(pass0, m_fourierX);
			else
				RTUtility.MultiTargetBlit(pass1, m_fourierX);
		}
		...
		for(i = 0; i < m_passes; i++, j++) 
		{
			...
			m_fourierY.SetTexture("_ButterFlyLookUp", m_butterflyLookupTable[i]);
			
			m_fourierY.SetTexture("_ReadBuffer0", data0[idx1]);
			m_fourierY.SetTexture("_ReadBuffer1", data1[idx1]);
			m_fourierY.SetTexture("_ReadBuffer2", data2[idx1]);
			
			if(idx == 0)
				RTUtility.MultiTargetBlit(pass0, m_fourierY);
			else
				RTUtility.MultiTargetBlit(pass1, m_fourierY);
		}
